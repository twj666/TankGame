...
import com.joker.tank.game.gameobject.bullet.general.PriMissileBullet;
...
*/
public class PriMissileFireStrategy implements FireStrategy {

    private int bulletNums;

    public PriMissileFireStrategy(int bulletNums) {
        this.bulletNums = bulletNums;
    }

    @Override
    public boolean addBulletNum(FireStrategy fireStrategy) {
        if (fireStrategy instanceof PriMissileFireStrategy) {
            this.bulletNums += ((PriMissileFireStrategy) fireStrategy).getBulletNums();
            return true;
        }
        return false;
    }
    ...
    public void fire(Tank tank) {
        switch (tank.getDir()) {
            case LEFT:
                new PriMissileBullet(tank, ResourceMgr.bullet_2_L);
                break;
            case UP:
                new PriMissileBullet(tank, ResourceMgr.bullet_2_U);
                break;
            case RIGHT:
                new PriMissileBullet(tank, ResourceMgr.bullet_2_R);
                break;
            case DOWN:
                new PriMissileBullet(tank, ResourceMgr.bullet_2_D);
                break;
            default:
                break;
        }
    ...
    public void paintFireStrategy(Graphics2D g, Rectangle rectangle) {
        ImageUtil.printImage(g, ResourceMgr.bullet_2_X, rectangle.x + 10 , rectangle.y + rectangle.height + 8);
