...
import com.joker.tank.game.gameobject.bullet.general.DefaultBullet;
...
public class DefaultFireStrategy implements FireStrategy {
    ...
    public DefaultFireStrategy(int bulletNums) {
        this.bulletNums = bulletNums;
    }

    @Override
    public boolean addBulletNum(FireStrategy fireStrategy) {
        if (fireStrategy instanceof DefaultFireStrategy) {
            this.bulletNums += ((DefaultFireStrategy) fireStrategy).getBulletNums();
            return true;
        }
        return false;
    }
    ...
    public void fire(Tank tank) {
        new DefaultBullet(tank, ResourceMgr.bullet_1);
        subBullet(tank);
    }

    @Override
    public void paintFireStrategy(Graphics2D g, Rectangle rectangle) {
        ImageUtil.printImage(g, ResourceMgr.bullet_1, rectangle.x + 10 , rectangle.y + rectangle.height + 8);
    ...
    public int getBulletWidth() {
        return ResourceMgr.bullet_1.getWidth();
    }
